...
import { labelPrintRoto, labelPrintRotoSearch, labelPrintMoldagemSearch } from "../../../services/apiLabels"
...
const EtitMoldagem = () => {
    ...
    const [moldagem, setMoldagem] = useState('');
    ...
    const handleBuscar =  async (e) => {
      ...
      try {
        const response = await labelPrintMoldagemSearch(texto);

        console.log(response)
        setCodigo(`${response.codigo} - 00`)
        setMaterial(`> ${response.material} <`);
        setMoldagem(`M ${response.moldagem}`);
        setAcabado((`A ${response.texto}`))
      } catch (error) {
        console.log(error);
      }
    }
  ...
  const isBotaoDesabilitado = texto === ''; // Verifica se o texto está vazio
    ...
    return (
    <section className="etiMoldagem">
      <div className="subTitulo">
        <h2>Etiquetas - Moldagem</h2>
      </div>
      ...
      <div className="corpoBloco">
        ...
        <div className="etiqueta etiqueta_para imprimir"
          ...
          <div className="item">
            ...
            <img src="../images/reciclavel.png" alt="sair" />
          </div>
        </div>
